...
- 状态：已实现 MCP server 内部 session record 管理、mock 验证和工具暴露

## 模块目标

Session Manager 用于在 MCP server 内部显式管理 AEDT/HFSS 会话记录，避免团队服务器上出现多个 AEDT 实例、多个工程或多个成员请求时误连错误会话。

该模块当前解决的是“显式会话记录与绑定”问题：所有连接 HFSS 的动作都会产生或复用一个 `session_id`，后续工具可以围绕这个 session 继续工作。真实 AEDT 进程扫描、真实进程启动和多 session broker 会在后续迭代中增强。
...
| `status` | `created`、`launched`、`connected`、`released` |
| `owner` | 请求方或团队成员标识 |
